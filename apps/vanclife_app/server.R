  ...
  # Determine empirical Bayesian estimates
    ...
    # Bind and order dose and concentration data
      input_data <- bind_rows(R.input.dose(), R.input.conc()) %>%
        ...
        mutate(AGE = input$covAge, WT = input$covWt, SCR = input$covScr) %>%
        mutate(HEEL = as.numeric(input$covHeel), INFDUR = input$covInf) %>%
        mutate(CANCER = as.numeric(input$covCanc))
...
# simulations, and therefore slower run-time of the application.
  ...
  # Create long-format simulation dataset
    ...
    # Extract optimised ETA values
      ...
      ETA_df <- as.data.frame(matrix(c(2, ETA), nrow = 1))
    ...
    # Define proportion and frequency of simulated concentrations for forecasting
      ...
      sim_freq <- 2  # sim_freq times between each dose
    ...
    # Add covariate data, ID = 2 is for individual predicted data
        mutate(ID = 2, AGE = input$covAge, WT = input$covWt) %>%
        mutate(SCR = input$covScr, HEEL = as.numeric(input$covHeel)) %>%
        mutate(CANCER = as.numeric(input$covCanc), INFDUR = input$covInf) %>%
