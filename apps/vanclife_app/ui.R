      ...
      # Dosing Input
  	  ...
  	  # Covariate Input
    			box(title = "Patient Data Input", width = 6, status = "primary",
    			  numericInput("covAge", "Age (years)",
    			    value = 35, min = 3.5, max = 100
    			  ),  # numericInput.patWt
    			  numericInput("covWt", "Weight (kg)",
    			    value = 70, min = 7, max = 700
    			  ),  # numericInput.patWt
    			  numericInput("covSecr", "Serum Creatinine (mg/dL)",
    			    value = 0.84, min = 0, max = 8.4
    			  ),  # numericInput.patSecr
    			  radioButtons("covHeel", "Finger-prick Sampling", inline = T,
    			    choices = list(
    			      "Yes" = 1,
    			      "No" = 0
    			    ),  # choices.list
    			    selected = 0
    			  ),  # radioButtons.patHeel
    			  radioButtons("covCanc", "Cancer", inline = T,
    			    choices = list(
    			      "Yes" = 1,
    			      "No" = 0
    			    ),  # choices.list
    			  ...
    			  numericInput("covInf", "Infusion Duration (hours)",
    			    value = 1, min = 0, max = 4
    			  )  # numericInput.patInf
    		  ),  # box.patient-input
