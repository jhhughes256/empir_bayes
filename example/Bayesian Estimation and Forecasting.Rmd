...
# Define bayesian estimation function
  ...
  # Parameters are defined as `exp(ETA)`, log transform to get ETA value
    ETA <- log(par)
  # The ETAs are then defined as a single row data.frame with ID for use later
    ETA_df <- as.data.frame(matrix(
      c(unique(single_tb$ID), ETA), 
      nrow = 1
    ))
  ...
  # Remove DV column, not needed for simulation
      dplyr::select(-DV) %>%
  # Merge in ETA data.frame using ID
...
```{r ebe6}
  ...
  # Create data.frame of Bayes estimates to merge into simulation input
    estim_ETA_df <- as.data.frame(matrix(
      c(unique(bayesin_tb$ID), log(bayes_estimate$par)), 
      nrow = 1
    ))
    names(estim_ETA_df) <- c("ID", paste0("ETA", 1:length(log(bayes_estimate$par))))
  # Create simulation input
    input_sim_tb <- bayesin_tb %>%
      dplyr::select(ID, time, evid, amt, cmt, rate, WT, DIAL, CRCL) %>%
    # Define ETAs as estimated
      inner_join(estim_ETA_df, by = "ID")  # merge in ETA data
